...
static void applyInversedSTransformation( // применение обратного S-преобразования
        uint8_t *block // входной блок размером в 16 байт
) { /* Аналогично S-преобразованию */
    for (int byteIndex_ = 0; byteIndex_ < BlockLengthInBytes; byteIndex_ += 8) {
...
static void applyInversedLTransformation(	// применение обратного преобразования L
        const uint8_t *input,				// входной блок размером в 16 (размер массива) байт
        uint8_t *output						// выходной блок (размер массива аналогичен входному)
) { /* Аналогично L-преобразованию */
    for (int byteIndex_ = 0; byteIndex_ < BlockLengthInBytes; ++byteIndex_) {
        uint8_t cache_ = 0;
        for (int addendIndex_ = 0; addendIndex_ < BlockLengthInBytes; ++addendIndex_) {
...
static void applyInversedSLXTransformation(	// применение обратного XSL-преобразования
        const uint8_t *key,					// входные ключи размером в 16 (размер массива) байт
        uint8_t *block,						// входной блок размером в 16 (размер массива) байт
        uint8_t *temporary					// вспомогательная память (размер массива аналогичен входному)
) { /* Аналогично XSL-преобразованию */
    applyXTransformation(key, block, temporary);
...
static void swapBlocks(
        uint8_t *__restrict left,
        uint8_t *__restrict right,
        uint8_t *__restrict temporary
) {
...
static void applyFTransformation(
        ...
        uint8_t *__restrict left,
        uint8_t *__restrict right,
        uint8_t *__restrict temporary1,
        uint8_t *__restrict temporary2
) {
...
static void fetchKeyScheduleRoundConstant(
        uint8_t index,
        uint8_t *__restrict roundConstant,
        uint8_t *__restrict temporary
) {
...
void encryptBlockWithGost15(
        const void *__restrict roundKeys,
        void * block
) {
...
void decryptBlockWithGost15(				// главная функция дешифрования
        const void *__restrict roundKeys,	// вычисленные раундовые ключи размером в 128 бит каждый
        void *__restrict block				// последовательность размером в 128 бит
) { /* Аналогично функции шифрования */
    const uint8_t *roundKeys_ = roundKeys;
    uint8_t cache_[BlockLengthInBytes] = {0};
