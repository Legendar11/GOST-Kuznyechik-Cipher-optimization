...
static void applyLSTransformation(	// Оптимальное преобразование LS
        const __m128i *input,		// входной блок (размер 128 бит)
        __m128i *output				// выходной блок (размер аналогичен)
) {
	...
	/* 16 операций сложения	с блоком из предвычисленной таблицы	*/
...
void encryptBlockWithGost15(				// Основной алгоритм шифрования
    ...
    data_ = _mm_loadu_si128(data); // загрузка операнда в регистр
    for (round_ = 0; round_ < NumberOfRounds - 1; ++round_) {
        cache_ = _mm_xor_si128(data_, *(const __m128i *) &roundKeys_[2 * round_]); // преобразование X
        applyLSTransformation(&cache_, &data_);	// преобразование LS
    }
    data_ = _mm_xor_si128(data_, *(const __m128i *) &roundKeys_[2 * round_]);
    _mm_store_si128(data, data_); // выгрука операнда из регистра
}
...
void decryptBlockWithGost15(				// Основной алгоритм дешифрования
    ...
    data_ = _mm_xor_si128(data_, *(const __m128i *) &roundKeys_[2 * (NumberOfRounds - 1)]); // преобразование X

    applySTransformation(&data_); // загрузка операнда в регистр
    applyInversedLSTransformation(&data_, &cache_); // обратное преобразование LS
    ...
    data_ = _mm_xor_si128(cache_, *(const __m128i *) &roundKeys_[0]);// преобразование X
    _mm_store_si128(data, data_); // выгрука операнда из регистра
}
